...
class PopupView(tk.Frame):
    ...
    def __init__(self, parent, close_popup_callback, refresh_callback):
    ...
    def _create_widgets(self):
        ...
        tk.Button(self, text="Coach", command=self._choose_coaches).grid(row=1, column=0)
        tk.Button(self, text="Player", command=self._choose_players).grid(row=1, column=1)
        tk.Label(self, text="Add New Member:").grid(row=0, columnspan=2, padx=10, pady=10)
    ...
    def _add(self):
        ...
        if self._players_or_coaches == 1:
            try:
                request = requests.post('http://127.0.0.1:5000/sports_team/member', json={
                ...
                if request.status_code != 200:
                    ...
                    tk.messagebox.showinfo('Success!', 'Entity Created with ID {}'.format(re.sub(r'[^1234567890]*', '', str(request.json()))))
            except Exception as e:
