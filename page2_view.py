...
class Page2View(tk.Frame):
    ...
    def __init__(self, parent, page_popup_callback, update_popup):
        """ Initialize Page 2 """
    ...
    def _create_widgets(self):
        ...
        tk.Button(self, text="Refresh", fg="Blue", command=self._refresh).grid(row=2, column=4)
        ...
        data_set = requests.get('http://127.0.0.1:5000/sports_team/member/all/player')
        if data_set.status_code != 200:
            return
        for item in data_set.json():
            self._listbox.insert(tk.END, "{} ({}) - {}".format(item["name"], item["id"], item["position"]))
